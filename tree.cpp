...
#include "tree.hpp"
using namespace std;
unsigned int Tree::m_vertexbuffer[3];
unsigned int Tree::m_texture;
int Tree::size;
/*
...
Tree::Tree()
{
    ...
    while(std::getline(File, Line)){
      ...
      if(Name == "v"){// Vertex
       ...
       // cout<<i<<" " <<tempdata[v1*3] <<" "<< " "<< tempdata[v1*3+1] << " " << tempdata[v1*3+2]<<endl;
        ...
        if(!texskip){
            texture.push_back(temptexture[t1*3]),texture.push_back(temptexture[t1*3+1]),texture.push_back(temptexture[t1*3+2]);
            texture.push_back(temptexture[t2*3]),texture.push_back(temptexture[t2*3+1]),texture.push_back(temptexture[t2*3+2]);
            texture.push_back(temptexture[t3*3]),texture.push_back(temptexture[t3*3+1]),texture.push_back(temptexture[t3*3+2]);
    ...
    CBitmap image("tree.bmp");               //read bitmap image
...
/*
 Destroy Tree - clean up geometry
 */
Tree::~Tree()
{
...
/*
 Draw Tree
 */
void Tree::drawTree(unsigned int m_program)
{
